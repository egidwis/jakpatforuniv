...
> ⚠️ **Folder ini berkali-kali terbukti basi sementara kodenya sudah mendarat.**
> Sebelum memperlakukan satu pun baris di bawah sebagai keadaan sekarang,
> verifikasi ke kode & produksi. Contoh terakhir: baris Phase 4 menyebut "empat
> dari lima penghalang masih berdiri" selama seminggu setelah kelimanya tumbang.
...
> ⚠️ **Empat** rencana di sini **bukan** bagian Jadwal Iklan.
>
...
>    `form_submissions_extend` selesai. Penahannya sudah lepas — **tapi strateginya diganti
>    2026-09-04**, lihat no. 4.
...
>    `sql/80`–`84`. **Dieksekusi, dideploy & seluruh migrasinya diterapkan 2026-09-03,
>    termasuk `sql/81`** (menyusul di hari yang sama). Efek terbesarnya bukan gerbang
>    pembatalan melainkan **piutang: Rp 18.772.750 → Rp 610.500**, karena 97%-nya ternyata
>    tagihan yang link DOKU-nya sudah mati. Pendapatan tidak bergerak.
>    Status berjalan di [§00X](../../jadwal-iklan-progress.md).
>    **Lanjutannya `sql/85` (2026-09-08)** — lihat baris "link bayar berwenang" di tabel
>    bawah. ⚠️ `85` **bertabrakan nomor** dengan migrasi AI pre-screening yang datang lewat
>    `main` di hari yang sama; keduanya sudah di produksi dan sengaja tidak diganti nama.
>    Duduk perkaranya di [`multi-step-form/sql/README.md`](../../../multi-step-form/sql/README.md).
> 4. [Transisi harga iklan Okt→Jan](2026-09-04-transisi-harga-iklan-okt-jan.md) **menggantikan
>    strategi** no. 2 dan **memakai ulang rancangan teknisnya**. Kenaikannya jadi bertahap
>    lewat "Harga Perkenalan" otomatis (1 Okt ±0% → 1 Des ≈+33% → 1 Jan 2027 harga penuh),
>    bukan sekali lompat yang dilunakkan halaman klaim voucher. Alasannya perilaku pelanggan,
>    bukan teknis: **±18% revenue duduk di SATU akun** unit kampus berdana anggaran, yang
>    gagalnya **sunyi** — ia tidak protes, ia cuma memesan lebih sedikit. Karena itu
>    pengumumannya harus mendahului musim penyusunan anggaran 2027.
...
> default. Guard Kilat `sql/42` tidak disentuh ~~dan tetap berlaku~~ — **guard itu DICABUT
> 2026-09-18 oleh `sql/95`** (Phase 5, lihat tabel di bawah): Kilat kini punya halaman.
> Kalimat "Guard Kilat tidak disentuh" di atas tetap benar **untuk `sql/55`**; ia hanya
> tidak lagi menggambarkan keadaan sekarang. **Committed di kode
...
>    ⚠️ **Premis chip itu terbalik 2026-09-18 (`sql/95`, Phase 5): Kilat SEKARANG punya
>    halaman, dan "Kilat lunas TANPA halaman" justru jadi anomali.** Chip-nya belum
>    disesuaikan — kalau menyentuh area ini, mulai dari sana.
...
|---|---|---|
| _(rencana di luar repo — sesi chat 2026-09-18)_ **Phase 5 — Kilat punya halaman, di luar `/pages`** | ✅ **`sql/95` diterapkan & diverifikasi 2026-09-18** · ✅ **dideploy & di-push 2026-09-18** (`e5b73bc` + `77d1e56`) · ⬜ uji manual browser belum dilakukan · 🟡 **2 order lunas tanpa tanggal menunggu keputusan bisnis** | **Membalik invarian yang ditegakkan di enam tempat sejak `sql/42`**: "order Kilat tidak boleh punya halaman". Kilat kini PUNYA halaman — ia tujuan pendaratan push notification, dan tautannya disalin admin dari dashboard JFU untuk dipasang ke dalam survei Jakpat. Alasan `sql/42` ("menerbitkan halaman memberi kapasitas tayang yang tidak dibayar") dinyatakan usang oleh pemilik produk: premisnya "halaman = kapasitas tayang" hanya benar bila halaman itu muncul di daftar publik — halaman Kilat tidak, disaring `distribution_type` di `/pages` dan `/api/surveys`. **`publish_end_date` Kilat sengaja `NULL`**: tautan yang sudah tertanam di survei Jakpat tidak bisa ditarik dari sisi kita, jadi ia tidak boleh punya tanggal mati; undian tetap tertutup lewat jalur terpisah (`get_batch_rewards_bulk` membaca `form_submissions.end_date`, **bukan** `publish_end_date`). ⚠️ **Karena `publish_end_date` NULL lolos filter tanggal, filter `distribution_type` adalah SATU-SATUNYA penahan** — tidak ada sabuk pengaman kedua; dikunci `surveyListingKilat.spec.ts`. Backfill melahirkan **17** halaman, bukan 19: **2 order lunas sejak awal Agustus punya `start_date`/`end_date` NULL** (juga NULL di `ad_schedules` — tanggalnya tidak pernah tersimpan di mana pun), gugur senyap di prasyarat `ensure_survey_page`. **Pra-existing, sengaja TIDAK diperbaiki**: mengisi tanggalnya = menjadwalkan ulang order berbayar tanpa diminta. Ikut mendarat: kanari dibalik (`countKilatPagesLeak` → `countKilatOrdersWithoutPage`), gerbang privasi peneliti di `getSurveyPagesBySubmissionIds`, tab Page admin dirombak jadi tiga keadaan, dan **perbaikan PII `/api/respondents`** sebagai commit terpisah: API key header-only (`?api_key=` dicabut — **breaking** untuk klien lama), CORS ditutup dengan sakelar env `RESPONDENT_API_ALLOWED_ORIGIN`, `is_hidden` dihormati di **kedua** mode. Payload data nol berubah. ⚠️ **Jendela tanggal sengaja TIDAK diperiksa** di endpoint itu — undian terjadi *sesudah* iklan berhenti tayang, menyaringnya akan mematikan undian semua survei. ⬜ **Chip `ScheduleEntryDrawer` menyisakan utang**: kalimat "tanpa halaman web landing page" sudah diperbaiki, tapi pembedaan chip lama masih mengasumsikan premis yang terbalik |
| [2026-09-10-phase-4-penjadwalan-swalayan](2026-09-10-phase-4-penjadwalan-swalayan.md) | ✅ **SELESAI & DITUTUP 2026-09-17** — dideploy, diuji browser, `sql/50`·`86`·`88`·`89`·`90` live | Penjadwalan swalayan di dashboard peneliti. **Peneliti menjadwalkan, membatalkan & memesan ulang perpanjangan sendiri tanpa admin** — terverifikasi di produksi. ⚠️ **"Kesiapan" di dalam dokumennya SUDAH BASI** (kelima penghalang 10 Sep sudah tumbang); ia dipertahankan sebagai arkeologi, bukan keadaan sekarang. ⚠️ Peneliti TIDAK bisa menukar jadwal yang masih berjalan — itu wewenang admin; yang ada adalah PESAN ULANG sesudah hangus. Pembatalan oleh peneliti kini mematikan link DOKU-nya (`f04ac21` + `sql/91`: `OWNER_ENDPOINTS` + penjaga kepemilikan penuh + policy/trigger kolom) — belum diuji browser. Utang tersisa: cron pelepas slot belum ada. Status: [§00AB](../../jadwal-iklan-progress.md) |
| _(rencana di luar repo)_ **link bayar berwenang — `/bayar/<id>`** · runbook: [`multi-step-form/docs/langkah-0-dan-9-doku.md`](../../../multi-step-form/docs/langkah-0-dan-9-doku.md) | ✅ **`sql/85` (8 Sep) + `sql/87` (10 Sep) diterapkan & diverifikasi** · ✅ **Rencana A dideploy, Rencana A-2 selesai & di-commit ke `main` 10 Sep** (`37d2234`, `7578a5c`, `3c86e12`) · ✅ **keenam uji browser lulus** · 🟡 **Cancel Order TIDAK PERNAH AKTIF di akun DOKU** — terjawab & **diaktifkan 10 Sep**; ⬜ belum ada satu pun pembatalan yang terbukti berhasil, dan DOKU membatasinya ke **VA & QRIS saja** · ✅ **3 email DOKU dimatikan 10 Sep** (Pesanan Berhasil & Gagal tetap nyala) | Link yang keluar dari sistem berhenti jadi URL DOKU mentah. Yang beredar sekarang `/bayar/<ad_schedules.id>`, dan pertanyaan "tagihan mana yang berwenang" dijawab **saat diklik** oleh `authoritative_payment_url()` (`sql/85`) — jadi link lama di inbox seseorang berhenti jadi masalah permanen. Menutup varian "**tanggalnya pindah**" dari insiden `af004b84`, yang [rencana 2026-09-03](2026-09-03-tagihan-mati-benar-benar-mati.md) tidak tutup (ia menutup varian "tagihannya dibatalkan"). Ikut mendarat: webhook mengenal tagihan **basi/tersalip** (`paid_on_stale_bill`, sengaja dipisah dari `paid_on_dead_bill` karena tindakan adminnya beda), `killDokuLink` di 4 titik de-otorisasi, gerbang kedaluwarsa di `InvoicePage`/`StatusPage`, dan invarian cutoff **13.00 / 14.00 / 60 menit** yang selama ini tidak tercatat di mana pun. ⚠️ **`sql/85` bertabrakan nomor** dengan `85_add_ai_prescreening_to_submissions` yang datang lewat `main` — keduanya sudah di produksi, tidak diganti nama; `86` tetap nomor bebas berikutnya dan tetap dipesan Phase 4. ⚠️ **Tiga percobaan Langkah 0 batal karena sebab di luar DOKU**: dua tagihan uji lahir tanpa `doku_request_id` (jembatan dev `vite.config.js` membuangnya — sudah diperbaiki), yang ketiga gagal karena `/api/doku/cancel-order` **tidak punya jembatan dev** sama sekali. Pembuktiannya karena itu menunggu deploy — dan **tidak menahannya**: perlindungan sesungguhnya ada di resolver, yang nol bergantung pada Cancel Order. **🔴 LANJUTAN 2026-09-10 — Rencana A-2 + `sql/87`.** Deploy Rencana A ternyata belum cukup: peneliti tetap terdampar, karena **empat** sebab berbeda — dan yang terbesar **regresi Rencana A sendiri** (`payLinkPath()` yang tak pernah `null` menggantikan `paymentUrl` yang bisa `null`, sementara nilai itu dipakai di hulu sebagai penanda keberadaan → setiap kartu selamanya menawarkan "Bayar Sekarang" untuk tagihan yang tidak ada). Ikut mendarat: 8 cadangan URL mentah dibuang (`invoices.schedule_id` kosong cuma 1 dari ~460 baris, dan NOL di antara tagihan hidup), origin `payLinkUrl` dikunci (apex `jakpatforuniv.com` menyajikan homepage statis → link email mati), email tagihan **perpanjangan** yang selama ini bisu total, dan halaman konfirmasi berhenti mencetak biaya ORDER sebagai "Total Pembayaran". `sql/87` menambah `reason = 'bill_cancelled'` (2 jadwal produksi yang hidup selama ini disuruh menjadwalkan ulang slot yang masih dipegangnya) dan kolom `doku_cancel_last_error` — **yang penolakan pertamanya langsung menjawab pertanyaan terbuka sejak awal: `Merchant not support cancel order, please do activation through DOKU dashboard`. Fiturnya tidak pernah aktif di akun ini; mencoba lagi tidak akan menolong.** Status berjalan di [§00Z](../../jadwal-iklan-progress.md) |
| [2026-09-04-transisi-harga-iklan-okt-jan](2026-09-04-transisi-harga-iklan-okt-jan.md) | ⬜ **belum dieksekusi — ditulis 2026-09-04.** Nol baris kode berubah · 🔴 **5 keputusan pemilik produk masih terbuka**, dua di antaranya memblokir Fase 3 | **Di luar alur Jadwal Iklan** — menggantikan **strategi** rencana 30 Agustus, memakai ulang **rancangan teknisnya**. Tarif naik ke **200/350/500/650/800** (+65,2%) tapi lewat **tiga langkah**: harga list baru tayang **1 Okt** dengan “Harga Perkenalan” −40% (efektif ±0%), −20% **1 Des** (≈+33%), harga penuh **1 Jan 2027**. Bantalannya **otomatis**, bukan halaman klaim voucher — halaman klaim punya *risiko penemuan* dan turun jadi lantai harga permanen segmen mahasiswa (Fase 4, tak lagi memblokir). Dua asumsi **dibatalkan pengukuran**: preseden kenaikan 2025 **buta** (`auth_user_id` cuma terisi 26,2% di Sep–Nov 2025, 6 order lunas beridentitas), dan **tier bukan proksi segmen** (T3 terbelah 48% pertama-kali / 52% berulang). Risiko sebenarnya **satu akun**: #1 = ±18% revenue, unit kampus berdana anggaran, gagalnya **sunyi**. ⚠️ **Temuan review kode 2026-09-04:** `SubmissionDetailSheet.tsx:576-577` **titik harga yang tak pernah terdaftar** · `SchedulePhase` **sudah bukan** titik harga (semua lewat `deriveScheduleMoney`) · `deriveScheduleMoney` juga kehilangan tanggal **voucher**, bukan cuma tarif · 🔴 **JFUFEB memakai cap Rp 300.000/hari yang hidup sampai 20 Feb 2027** — terhadap tangga baru itu jadi diskon **40–62%** yang tak pernah diputuskan siapa pun (7 order, 6 lunas; ILKOMUNY nol pemakaian seumur hidup) |
| [2026-09-03-tagihan-mati-benar-benar-mati](2026-09-03-tagihan-mati-benar-benar-mati.md) | ✅ **dieksekusi & DIDEPLOY 2026-09-03** — `sql/80`·`81`·`82`·`83`·`84` semuanya diterapkan & diverifikasi · ⬜ Cancel Order API belum diuji di sandbox | **Di luar alur Jadwal Iklan** — insiden pembayaran order `af004b84`: uang mendarat di link DOKU milik jadwal yang sudah dibatalkan 20 menit sesudah tagihannya terbit. Tiga asumsi hulu yang gagal: `is_stale` buta terhadap jadwal yang *mati* (hanya melihat yang *pindah*, karena `cancelSchedule()` mempertahankan tanggal); umur link dipatok 7 hari; dan "uang masuk boleh diterapkan ke jadwalnya" — benar untuk **buku besar**, tidak untuk **jadwal**. Sekarang: outcome `paid_on_dead_bill` (nol tulisan, 200, antrean admin), umur link mengikuti batas bayar jadwal dengan lantai 60 menit yang **menolak** alih-alih meng-clamp, gerbang "batalkan tagihannya dulu", dan Cancel Order API DOKU. ⚠️ **Menukar kegagalan berisik dengan kegagalan sunyi DI BUKU** — pendapatan kurang hitung sampai admin bertindak; rekonsiliasi harus tahu kelas selisih ini ada. **`sql/81` menurunkan piutang berjalan Rp 18.772.750 → Rp 610.500** — 97%-nya ternyata tagihan yang link DOKU-nya sudah mati; pendapatan tidak bergerak, nol baris lunas tersentuh. (Dampak gerbangnya sendiri kecil: 11 jadwal, bukan 75 — angka awal salah hitung, lihat §00X.) Temuan di luar rencana: `transactions` **tidak punya** `expires_at`; **tidak ada cron** yang mengedaluwarsakan tagihan (182/183 baris `pending` sudah lewat 7 hari, tertua Des 2025); tiga outcome `sql/77` tidak pernah masuk peta label banner. Status berjalan di [§00X](../../jadwal-iklan-progress.md) |
| [2026-08-30-kenaikan-harga-voucher-klaim](2026-08-30-kenaikan-harga-voucher-klaim.md) | 🟡 **strateginya DIGANTI 2026-09-04** oleh [transisi harga Okt→Jan](2026-09-04-transisi-harga-iklan-okt-jan.md) — baca koreksi di kepalanya dulu. Rancangan teknis & angka dampaknya **tetap berlaku penuh**; yang batal urutan & bentuk bantalannya. Nol baris kode berubah | Tangga tarif naik ke **200/350/500/650/800** (kesepakatan 2026-07-20 yang tak pernah dijalankan) — **+65,2%** atas bauran order nyata, ±Rp 29,8jt → 49,2jt/bulan. Dilunakkan **halaman "Klaim Voucher" self-serve**: potongan persen bercap, sekali per akun, bermasa berlaku. JFUSUHUD dibiarkan mati 31 Agu 2026 dan voucher klaim jadi penggantinya. ⚠️ **`calculateAdCostPerDay()` tidak punya parameter tanggal** sementara `create-payment.js` menghitung ulang harga tiap kali orang menekan bayar — menaikkan tarif begitu saja akan menagih **80 order approved-belum-bayar** rata-rata **+Rp 230.000** (total +Rp 18,4jt) tanpa ada yang memberi tahu. Karena itu tangga tarif dibuat **ber-tanggal**, dan Fase 1 dikirim sebagai **no-op**. ⚠️ Titik harga ada **TIGA**, bukan empat — `StepOneFormFields` itu tangga *hadiah responden*, bukan tarif iklan |
...
> ⚠️ **URUTAN [1]–[4] DI ATAS SUDAH TERLAMPAUI (17 Sep 2026).** Phase 4 sudah
> dieksekusi dan diuji di browser; Task 13 sudah selesai. Blok di atas dan tiga
> alasan di bawah dipertahankan sebagai **arkeologi keputusan 2026-08-18**, bukan
> sebagai urutan yang berlaku. Khususnya: "tidak ada harga untuk jadwal ke-2"
> sudah lunas — harga lahir di `create-payment.js` dan `total_cost = 0` pada baris
> swalayan memang disengaja, dan `reward_pools` sudah ada berisi 900 baris.
> Keadaan sekarang: [§00AB](../../jadwal-iklan-progress.md).
>
> **18 Sep 2026 — Phase 5 menyusul dan sudah dideploy** (`sql/95`, commit `e5b73bc`
> + `77d1e56`). Ia tidak ada dalam urutan [1]–[4] karena lahir sesudahnya, dari
> sesi chat, bukan dari rencana pra-tulis. Perubahan paling berdampak untuk siapa
> pun yang menyentuh `survey_pages`: **halaman Kilat kini ADA dan terbit**, dan
> yang menahannya keluar dari daftar publik hanya filter `distribution_type` di
> dua permukaan. Rinciannya di tabel.
