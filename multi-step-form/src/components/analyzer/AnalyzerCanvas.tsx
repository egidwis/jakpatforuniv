...
export const AnalyzerCanvas: React.FC<AnalyzerCanvasProps> = ({
  ...
  const handleCopyText = (id: string, text: string) => {
    navigator.clipboard.writeText(text)
      .then(() => {
        setCopiedId(id);
        toast.success('Narasi disalin ke clipboard');
            ...
            <div className="flex items-center gap-1 opacity-80 group-hover:opacity-100 transition-opacity">
              {block.narrative && (
                <button
                  ...
                  title="Copy Narasi untuk Skripsi"
                  className="p-1.5 rounded-lg hover:bg-gray-100 text-gray-500 hover:text-indigo-600 transition-colors"
                >
                  {copiedId === block.id ? <Check className="w-4 h-4 text-emerald-600" /> : <Copy className="w-4 h-4" />}
                </button>
              )}
            ...
            <div className="mt-3.5 pt-3.5 border-t border-gray-100 bg-indigo-50/30 -mx-5 -mb-5 md:-mx-6 md:-mb-6 p-4 rounded-b-2xl">
              <div className="flex items-center justify-between mb-1.5">
                ...
                >
                  ...
                  {copiedId === block.id ? 'Tersalin' : 'Copy Narasi'}
                </button>
              </div>
